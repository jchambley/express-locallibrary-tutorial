    ...
    (req, res, next) => {
        ...
        var bookinstance = new BookInstance(
          { book: req.body.book,
            ...
            status: req.body.status,
            due_back: req.body.due_back,
	    _id: req.params.id //use the _id, since this is an update edit.	    
           });
        ...
        if (!errors.isEmpty()) {
            ...
            Book.find({},'title')
                .exec(function (err, books) {
                    if (err) { return next(err); }
                    // Successful, so render.
                    res.render('bookinstance_form', { title: 'Update Book Copy Information', book_list: books, selected_book: bookinstance.book._id , errors: errors.array(), bookinstance: bookinstance });
            });
            return;
        }
        else {
